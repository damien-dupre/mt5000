...
GitHub has a lot of different functions. For now, we will only see how to use it to published the html document output from out Quarto file.
...
8/ Below the commit message fields, decide whether to add your commit to the current branch or to a new branch. Select **commit directly to the `main` branch** for now. For collaborative projects never commit to the main branch
...
3/ Click on **Actions** and wait that "pages build and deployment" finishes. When it's done, go to **https://yourusername.github.io/yourrepositoryname/**
