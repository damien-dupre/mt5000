...
From the first (bottom) to the last (top), the layers are :
...
in its `aes()`. Use a theme of your choice as well (e.g., `theme_bw()`, `theme_classic()`, `theme_minimal()`, `theme_dark()`):
...
You can add as many geometry layers as you want, however repeating the mapping for each geometry layer is very redundant. Thankfully, if all your geometry layers are using the same aesthetics mapping, __it is possible to include this mapping inside the `ggplot()`__, then all the geometry layers will have the same mapping:
...
ggplot(data = map_pop_2007, aes(x = long, y = lat)) +
  geom_polygon(aes(group = group, fill = pop))
```
